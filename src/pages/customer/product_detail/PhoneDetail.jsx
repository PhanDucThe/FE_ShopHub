...
const PhoneDetail = () => {
  ...
  const getCurrentVariant = () => {
    ...
    const storageOption = products?.availableOptions?.Storage?.find(
      (storage) => storage.slug === selectedStorage
    );

    // Tìm color trong storage đã chọn
    const colorOption = storageOption?.Color?.find(
        ...
        <div className="max-w-7xl mx-auto px-4 py-1">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-6">
              ...
              {/* Action Buttons */}
              <ActionButton
                variantId={products?.currentVariant?.variantId}
                quantity={1}
              />
              {/* Services */}
              <Services />
